...
function Field({
  ...
  // Per-field visibility toggle, only meaningful when type starts as
  // "password". Held locally so each password field tracks its own
  // state independently — toggling one doesn't reveal the other.
  const [visible, setVisible] = useState(false);
  const isPassword = type === "password";
  const effectiveType = isPassword && visible ? "text" : type;

  // Make room on the right for the eye button when this is a password.
  // Larger padding for the icon column so text doesn't slide under it.
  const inputClass = `h-11 w-full rounded-xl border border-border bg-surface text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:border-accent focus:ring-2 focus:ring-accent/20 ${
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="absolute inset-y-0 right-0 flex w-11 cursor-pointer items-center justify-center text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:text-foreground"
      >
        {visible ? (
          // Eye-off — open eye with a slash through it
          <svg
            className="h-4 w-4"
            fill="none"
            viewBox="0 0 24 24"
